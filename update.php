<?php
    ...
    if( strcmp($command,"drag") == 0 ) {
        ...
        $query = "UPDATE `".TABLE_PREFIX."at_cal_full_calendar_events` SET start = '".$newstartd.
        ...
        mysql_query( $query, $db );
        $query = "SELECT MAX(id) from `".TABLE_PREFIX."at_cal_full_calendar_events`";
        ...
        //Delete the event
        $query = "DELETE FROM `".TABLE_PREFIX."at_cal_full_calendar_events` WHERE id=".$iddd;
        mysql_query( $query, $db );
    }
    else if( strcmp($command,"update") == 0 ) {
        //User clicked on the event. The dates and title may be changed. So update the database record.
        $query = "UPDATE `".TABLE_PREFIX."at_cal_full_calendar_events` SET title = '".$newtitle."' , start = '".
